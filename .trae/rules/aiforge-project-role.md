...
## 3. Python AI Agent Specifications (FastAPI & LangChain)
- **Tech Stack**: Python 3 + FastAPI + LangChain | Pydantic for data validation.
- **Global Constraint**: Strictly follow the layered architecture. Cross-layer bypasses are strictly forbidden. Always use `async/await` for I/O-bound operations (database queries, external APIs, LLM calls).
- **Module Responsibilities**:
  - `app/`: API routing (`api.py`), DTO assembly, and dependency injection (`dependencies.py` for DB sessions, auth).
  - `services/`: Business rules orchestration, pre/post-processing, and coordination between `models` and `agents`.
  - `agents/`: Core AI logic, LangChain orchestration, Prompt engineering, memory management, and LLM interaction.
  - `models/`: Data persistence and ORM.
- **Coding Standards & AI Output Rules**:
  - **Strict Typing (CRITICAL)**: All functions and methods MUST include complete Python Type Hints.
  - **Object Layering**: Exclusively use `pydantic.BaseModel` for data validation and transfer. All API inputs/outputs must be defined in `app/schemas.py`. Input classes MUST end with the `Request` suffix (e.g., `ChatRequest`). Output classes MUST end with the `Response` suffix (e.g., `ChatResponse`). NEVER use raw `dict` or `str` as request bodies.
  - **Unified Response**: All API endpoints MUST return a standardized JSON structure (e.g., `{"code": 200, "message": "success", "data": {...}}`).
  - **Logging & Exceptions**: Use the standard Python `logging` module (or `loguru`). NEVER use `print()`. Do not return error dictionaries; instead, raise custom exceptions (defined in `app/exceptions.py`) or `HTTPException` to be intercepted by the global exception handler.
  - **Naming Conventions**: Variables and functions must use `snake_case`. Class names must use `PascalCase`. Imports MUST be absolute (e.g., `from app.schemas import ChatRequest`).
  - **Configuration**: NEVER hardcode settings. Strictly use `pydantic-settings` via `app/config.py` to read environment variables.
